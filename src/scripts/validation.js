...
export function showInputError(formElement, inputElement, errorMsg, settings) {
  ...
  if (!errorMsgEl) {
    console.warn(`showInputError: missing error element for input "${inputElement.id}"`);
    return;
  }
  errorMsgEl.textContent = errorMsg || inputElement.validationMessage || '';
...
export function hideInputError(formElement, inputElement, settings) {
  const errorMsgEl = formElement.querySelector(`#${inputElement.id}-error`);
  if (!errorMsgEl) {
    console.warn(`hideInputError: missing error element for input "${inputElement.id}"`);
    return;
  }
